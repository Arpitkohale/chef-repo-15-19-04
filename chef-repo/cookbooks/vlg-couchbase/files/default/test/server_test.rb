describe_recipe "vlg-couchbase::server" do
  ...
  describe "log directory" do
    let(:log_dir) { directory node['vlg-couchbase']['server']['log_dir'] }
  ...
  describe "static_config" do
    let(:static_config) { file File.join(node['vlg-couchbase']['server']['install_dir'], 'etc', 'couchbase', 'static_config') }

    it "moves the log directory" do
      static_config.must_include %{{error_logger_mf_dir, "#{node['vlg-couchbase']['server']['log_dir']}"}.}
      static_config.wont_match /error_logger_mf_dir.*error_logger_mf_dir/
    end
  end

  describe "database directory" do
    let(:database_dir) { directory node['vlg-couchbase']['server']['database_path'] }
  ...
  describe "index directory" do
    let(:index_dir) { directory node['vlg-couchbase']['server']['index_path'] }
  ...
  describe "self Couchbase node" do
    ...
    it "has its database path configured" do
      node_self.must_have :database_path, node["vlg-couchbase"]["server"]["database_path"]
    end

    it "has its index path configured" do
      node_self.must_have :index_path, node["vlg-couchbase"]["server"]["index_path"]
    end
  end
  ...
  describe "default Couchbase cluster" do
    ...
    it "has its memory quota configured" do
      cluster.must_have :memory_quota_mb, node["vlg-couchbase"]["server"]["memory_quota_mb"]
    end
  end
end
