...
include '/var/www/pd2jira/var.php';
$jira_url = "https://videologygroup.atlassian.net";
$jira_username = $username; 
$jira_password = $password;
$jira_project = "PDD";
$jira_issue_type = "Task";
$jira_issue_priority = "High";
$pd_subdomain = "videology";
$pd_api_token = $api_token;
  ...
  switch ($webhook_type) {
    case "incident.trigger":
      ...
      $service_name = $webhook->data->incident->service->name;
          $rservice = str_replace(' ', '_', $service_name);
      ...
      $data = array('fields'=>array('project'=>array('key'=>"$jira_project"),'summary'=>"$summary",'description'=>"A new PagerDuty ticket as been created.  {$trigger_summary_data}. Please go to $ticket_url to view it.
          assignee to it is $assignee", 'issuetype'=>array('name'=>"$jira_issue_type")));
      $data_json = json_encode($data);

      $return = http_request($url, $data_json, "POST", "basic", $jira_username, $jira_password, $service_name);
      ...
      if ($status_code == "201") {
        //Update the PagerDuty ticket with the JIRA ticket information.
                 $crl = "curl -u '$jira_username:$jira_password' -H \"Content-Type: application/json\" -X PUT --data '{ \"update\": { \"labels\": [ {\"add\": \"$rservice\"} ] } }' $jira_url/rest/api/2/issue/$response_key";
                exec ($crl);
                $url = "https://$pd_subdomain.pagerduty.com/api/v1/incidents/$incident_id/notes";
...
function http_request($url, $data_json, $method, $auth_type, $username, $token, $service) {
  $ch = curl_init();
